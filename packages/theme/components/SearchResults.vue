<template>
  <div>
      ...
      <transition-group name="sf-fade" class="check-w" mode="out-in">
        ...
        <div v-else-if="catalogs && catalogs.length > 0" class="search__wrapper-results" key="results">
          <div class="h-padding result-num">
            <span><span v-e2e="'total-result'">{{ totalResults(catalogs) }}</span>
              results found</span>
          </div>
          <div v-for="(bpp, bppIndex) in catalogs" :key="bppIndex">
            <div v-for="(provider, prIndex) in bpp.bpp_providers" :key="prIndex">
              ...
              <div class="results--mobile">
                <ProductCard v-for="(product, pIndex) in provider.items"
                  @goToProduct="goToProduct(product, provider, bpp)" :key="bppIndex +
                    ...
                    keyVal +
                    'product'
                    " :pIndex="pIndex" :pName="productGetters.getName(product)"
    ...
    <div v-if="cart.totalItems" class="sr-footer">
      <Footer @buttonClick="footerClick" :totalPrice="cart.totalPrice" :totalItem="cart.totalItems"
